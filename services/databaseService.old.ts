...
class DatabaseService {
  ...
  async createUser(name: string, metrics: UserMetrics, preferences: UserPreferences, email?: string): Promise<number> {
    try {
      const result = db.runSync(
        ...
        [name, email || null, JSON.stringify(metrics), JSON.stringify(preferences)]
      );
  ...
  async updateUser(userId: number, updates: Partial<Omit<DatabaseUser, 'id' | 'created_at'>>): Promise<void> {
    try {
  ...
  async updateUserProgress(userId: number, updates: Partial<Omit<DatabaseUserProgress, 'id' | 'user_id'>>): Promise<void> {
    try {
